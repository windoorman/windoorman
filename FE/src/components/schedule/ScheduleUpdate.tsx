...
const ScheduleUpdate = () => {
  ...
  const { groupId, windowsId, startTime, endTime, days } = location.state || {};

  const [selectedDays, setSelectedDays] = useState<string[]>(days || []);
  const [startHour, setStartHour] = useState(startTime?.split(":")[0] || "07");
  const [startMinute, setStartMinute] = useState(
    startTime?.split(":")[1] || "00"
  );
  const [endHour, setEndHour] = useState(endTime?.split(":")[0] || "07");
  const [endMinute, setEndMinute] = useState(endTime?.split(":")[1] || "15");
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setModalMessage("일정 수정에 실패했습니다. 다시 시도해 주세요.");
      setShowModal(true);
    }
  };
